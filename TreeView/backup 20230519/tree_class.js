...
class TreeView{
    constructor(instance_name, tree_div_id, title, branch_selected_cb = null, branch_hover_cb = null, branch_rightclick_cb = null){  
        ...
        this.branch_rightclick_cb = branch_rightclick_cb;

        // Specify if the selected or hover bar are displayed
        this.showSelectionBar = true;
        this.showHoverBar = true;
        ...
        // Define a timer for the info pop-up
        this.infoTimer = setTimeout(null, 1);
        this.infoTimeout = 1000; // 1000 milliseconds = 1 second
        ...
        this.iconSvgs.push(folder_icon);
        this.iconSvgs.push(document_icon);
    ...
    setDarkMode(){
        ...
        this.selectedTextColor = "#cacbcc";
        this.hoverOverColor = "#2a2d2e";
        this.hoverOverTextColor = "#cacbcc";
        document.getElementById(this.tree_div_id).style.color = "#cacbcc";
        document.getElementById(this.tree_div_id).style.backgroundColor = "#252526";
        this.iconColor = "#cacbcc";
        this.hoverAwayColor = "rgb(0,0,0,0)";
    ...
    append(uid, label, type, info = undefined, userObj = {}){
        ...
        if(index === undefined){
            ...
            let new_branch = {expanded:false, selected:false, type:type, level:0,label:label,uid:new_uid,info:info,user:userObj};
            ...
            let new_branch = {expanded:false, selected:false, type:type, level:level,label:label,uid:new_uid,info:info,user:userObj};
    ...
    appendChildAtSelection(label, type, info = undefined, userObj = {}, expand = true){
        ...
        let new_uid = this.append(selected_uid, label, type, info, userObj);
        return new_uid;          
    }
    ...
    branchClick(elm, dblClick = false){
        ...
        // Call user to inform that a branch was selected. Provides branch uid and user object as argument
    ...
    branchMouseOver(elm){
        ...
        // Populate and make info pop-up visible
        let info_elm = document.getElementById(this.tree_div_id+"-info"); 

        // Find the absolute position of the branch
        let pos = elm.getBoundingClientRect();

        let info_text = this.list[this.findBranchIdx(elm.getAttribute("id"))].info
        
        // Only show the info pop-up if defined and also not an empty string
        if(info_text !== undefined && info_text != ""){
            
            // Calculate the horizontal position of the pop-up
            let hor_pos = pos.x;
            hor_pos += 5 * this.fontSizeNumber;
            hor_pos += this.list[this.findBranchIdx(elm.getAttribute("id"))].level*this.indentMultiplier*this.fontSizeNumber;
            ...
            this.infoTimer = setTimeout(function() {
                info_elm.style.display = "inline";
            }, this.infoTimeout);
        }
                
        // Callback
        let branchIdx = this.findBranchIdx(elm.getAttribute("id"));
        if (this.branch_hover_cb != null)
    ...
    branchMouseLeave(elm){
        // Clear the info pop-up timer if it is currently running
        clearTimeout(this.infoTimer);
    ...
    drawTree(){
        ...
        if(this.title !== undefined && this.title != ""){
            ...
            title_div.innerHTML = "<b>" + this.title + "</b>";
            title_div.style.paddingBottom = "" + this.fontSizeNumber/4 +"px";
            title_div.addEventListener('mousedown', function(e){ e.preventDefault(); }, false);
            document.getElementById(this.tree_div_id).appendChild(title_div);
        }

        // Insert an invisible info DIV (will be postitioned and populated when user hovers over a branch)     
        let info_style = "display:none; border-radius: 0px 4px 4px 4px; padding-left: 5px; padding-right: 5px; position : fixed; border: solid black 1px; font-family: Verdana, sans-serif; font-size: 14px;";
        let info_div = document.createElement('div');
        info_div.style = info_style;
        info_div.id = this.tree_div_id + "-info";
        info_div.display = "none";
        info_div.style.backgroundColor = this.infoBoxColor;
        info_div.style.color = this.infoTextColor;
        document.getElementById(this.tree_div_id).appendChild(info_div);
        ...
        for(let i=0; i < this.list.length; i++){
            ...
            if(expansion_list.slice(-1)=="show"){
                ...
                // Find the required type icon and adjust the size (width and height is identical to font size used)
                let type_icon = this.iconSvgs[branch.type];
                type_icon = type_icon.replace("$fontSize",this.fontSizeNumber.toString());
                type_icon = type_icon.replace("$fontSize",this.fontSizeNumber.toString());
                ...
                // Register hover functions if enabled
